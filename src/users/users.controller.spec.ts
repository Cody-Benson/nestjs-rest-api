...
import { createTestUser, createTestUserDTO, createTestUsers } from "../../test/utils/user-test.util";
...
describe('UserController test suite',()=>{
    ...
    it('should return users',async ()=>{
        const expected = await Promise.resolve(createTestUsers(2));
        expect(await userController.getUsers()).toEqual(expected);
    });

    it('should create a new user',async ()=>{
        const expected:User = await Promise.resolve(createTestUser());
        const createUserDTO:CreateUserDTO = createTestUserDTO();
